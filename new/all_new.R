...
##Bias_p fun
Bias_p<-function(data,whether_riskline,risk_line1,risk_line2,date_interval_set){
...
#Kd_p fun
Kdj_p<-function(data,date_interval_set){
...
#Rsi_p fun
Rsi_p <- function (data,date_interval_set){
  #將資訊輸入
  sdata_f<-data
  snum<-sdata_f[[1]]
...
#Obv_p fun
Obv_p<-function(data,date_interval_set){
...
#Arbr_p fun
Arbr_p<-function(data,date_interval_set){
  #將資訊輸入
  sdata_f<-data
  snum<-sdata_f[[1]]
...
stock_data_TWII<-function(snum,fromdate,todate){
  ...
  ##原矩陣
  #from為起始時間 #Sys.Date()
  sdata <- getSymbols(paste0('^',"TWII"),auto.assign=FALSE,src="yahoo",from=fromdate,to=todate)
...
stock_data_DJI<-function(snum,fromdate,todate){
  ...
  ##原矩陣
  #from為起始時間 #Sys.Date()
  sdata <- getSymbols(paste0('^',"DJI"),auto.assign=FALSE,src="yahoo",from=fromdate,to=todate)
...
box_p1<-function(data){
  ...
  colnames(data) <- c("開盤價", "最高價", "最低價", "收盤價", "調整價")
  boxplot(data)
  ...
  navbarPage(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          tabsetPanel(#根據不同條件變更ui
            ...
            tabPanel("RSI", 
                     numericInput("min", "最小值", value = 0),
                     numericInput("max", "最大值", value = 1)
            ),
            tabPanel("OBV",
                     numericInput("rate", "比率", value = 1, min = 0),
            ),
            tabPanel("AR&BR",
                     numericInput("rate", "比率", value = 1, min = 0),
            ),
        ...
        mainPanel(
          tabsetPanel(
            ...
            tabPanel("台/美股票大盤",
                     br(),div("布林通道",style = "color:orange"),
                     box(
                       title = "", status = "primary", solidHeader = TRUE,width =NULL,height = NULL,
                       withLoader( plotOutput("TWII_BB"), type="html", loader="pacman")
                     ),
                     box(
                       title = "", status = "primary", solidHeader = TRUE,width =NULL,height = NULL,
                       withLoader( plotOutput("DJI_BB"), type="html", loader="pacman")
                     ),
                     br(),div("移動平均線",style = "color:orange"),
                     box(
                       title = "", status = "primary", solidHeader = TRUE,width =NULL,height = NULL,
                       withLoader( plotOutput("TWII_MA"), type="html", loader="pacman")
                     ),
                     box(
                       title = "", status = "primary", solidHeader = TRUE,width =NULL,height = NULL,
                       withLoader( plotOutput("DJI_MA"), type="html", loader="pacman")
                     )
            ...
            tabPanel("彙整表",
                     br(),
                     tags$li("開盤價 : 開盤時(股票市場開始營業時)的股票價格"),
                     tags$li("收盤價 : 收盤時(股票市場關門打烊時)的股票價格"),
                     tags$li("最高價 : 一天中最高的成交價"),
                     tags$li("最低價 : 一天中最低的成交價"),
                     tags$li("成交量 : 某一特定資產在一段時間內交易的數量"),
                     box(
                       title = "", status = "primary", solidHeader = TRUE,width =NULL,height = NULL,
                     ...
                     box(
                       title = "", status = "primary", solidHeader = TRUE,width =NULL,height = NULL,
                       withLoader( plotOutput("box_p1"), type="html", loader="pacman")
                     ),box(
                       title = "", status = "primary", solidHeader = TRUE,width =NULL,height = NULL,
                       withLoader( plotOutput("box_p2"), type="html", loader="pacman")
                     )
            )
          )
        )
    ...
    navbarMenu(
      ...
      tabPanel("指標技術應用", value = "about",
               ...
               tags$li("正乖離：收盤價 > 移動平均價 →股價高於均價，股價下跌修正的可能性變高負乖離：收盤價 < 移動平均價→股價低於均價，股價上漲修正的可能性變高"),
      ...
      tabPanel("指標教學圖", value = "about",
               mainPanel(
                 ...
                 div(tags$img(src="ma.png",height="350px",width="750px",alt="Something went wrong")),
                 h5("BBands 布林通道", style = "color:white;"),
                 div(tags$img(src="bb.jpg",height="400px",width="900px",alt="Something went wrong")),
                 h5("BISA 乖離率", style = "color:white;"),
                 div(tags$img(src="bias.png",height="350px",width="750px",alt="Something went wrong")),
                 h5("KDJ 隨機指標", style = "color:white;"),
                 div(tags$img(src="kdj.jpg",height="540px",width="800px",align ="Something went wrong")),
                 h5("RSI 相對強弱指數", style = "color:white;"),
                 div(tags$img(src="rsi.png",height="500px",width="900px",align = "Something went wrong")),
                 h5("OBV 能量潮指標", style = "color:white;"),
                 div(tags$img(src="obv.png",height="600px",width="600px",align = "Something went wrong")),
                 h5("AR開價指標", style = "color:white;"),
                 div(tags$img(src="ar.png",height="400px",width="1000px",align = "Something went wrong")),
                 h5("BR收盤價強弱指標", style = "color:white;"),
                 div(tags$img(src="br.png",height="400px",width="1000px",align = "Something went wrong"))
               )
      )),
    #分享
    navbarMenu(
...
server = function(input,output,session) {
  ...
  react_valid_dateorder_abbr<- reactive({
    validate(need(input$date[2]>input$date[1] , "請檢查時間順序是否輸入錯誤"),
             need(!is.numeric(react_stocknum_str),"請檢查輸入股票代碼時是否加入臺灣市場縮寫(.TW)")
    )
  })
  ##########################
  ...
  react_index_plot <- reactive({
    switch(input$index1,
           "BIAS" = Bias_p(react_sdata(),input$whether_riskline,
                           input$up_bias_risk,input$dn_bias_risk,
                           react_dates_interval_str()
           ),
           "RSI" = Rsi_p(react_sdata(),react_dates_interval_str()),
           "OBV" = Obv_p(react_sdata(),react_dates_interval_str()),
           "AR&BR" = Arbr_p(react_sdata(),react_dates_interval_str()),
           "KDJ" = Kdj_p(react_sdata(),react_dates_interval_str())
    )
  })
  ...
  output$smtable = DT::renderDataTable({
    ...
    DT::datatable(react_fun$temcanpdf , options = list(pageLength = 25))
  })
  ...
  ##台股大盤
  output$TWII_BB= renderPlot({
    #
    TWII_BB_p(react_sdata_TWII())
    
  })
  output$TWII_MA= renderPlot({
    #
    TWII_MA_p(react_sdata_TWII())
    
  })
  
  ##美股大盤
  output$DJI_BB= renderPlot({
    #
    DJI_BB_p(react_sdata_DJI())
  })
  output$DJI_MA= renderPlot({
    #
    DJI_MA_p(react_sdata_DJI())
  })
